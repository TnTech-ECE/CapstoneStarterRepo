...
| 4   | Shall be able to send audio data and receive filter data (a full cycle) within a 4 second frame | Design Constraint |
...
The Blackfin ADSP-BF70x EZ-KIT Mini is built for direct connectivity to an Arduino UNO or related board. The board has a corresponding through-hole for each input/output port of the Arduino UNO. It is able to directly stack on top using 0.1" header pins. The Arduino UNO R4 WiFi has the same pinout and form as the Arduino UNO R3. This will allow the main processing subsystem to communicate via SPI with the Bluetooth connection subsystem<sup>3</sup>. The Arduino SPI library will allow easier programming of the communication. Since 2.20 seconds is used during the Bluetooth cycle, 1.80 seconds is left for SPI communication. The full 3.072 Mb (1.536 Mb * (1 delay line + 1 filter line)) divided by 1.80 seconds gives us a minimum SPI rate of 1.707 Mbps. The RA4M1 has a clock speed of 48 MHz[2]. With SPI, the theoretical max data rate is 24 Mbps. But more accurately, 16 or 8 Mbps is often used<sup>4</sup>.
