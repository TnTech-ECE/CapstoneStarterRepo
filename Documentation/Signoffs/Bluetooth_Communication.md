...
| 5   | Shall be able to store all pre-trained filters                        | Design Constraint |
| 6   | Bluetooth's range must reach anywhere in the room                     | Design Constraint |
| 7   | Shall not store audio data after processing                           | Ethics Constraint |
| 8   | Shall encrypt any data sent wirelessly                                | Ethics Constraint |
| 9   | Must not add any physical obstructions through the walking area of the room       | Ethics Constraint |
...
<sup>2</sup> Each window will require at least one DSP board in order to allow speedy calculations. Since the machine learning aspect of the adapting algorithm is computationally expensive, data is passed between the main processor subsystem and the machine learning subsystem. Since the machine learning subsystem's purpose is not to solve the "optimimum" filter, but to choose from a set of pre-trained filters based on frequency (not dynamic range), our data being sent can be compressed without greatly effecting system performance. 8 bits is then used as a balance. The system therefore must send the raw audio data (compressed to most significant 8 bit samples) to the machine learning subsystem and receive the trained filter coefficients (compressed to most significant 8 bit samples).
...
<sup>4</sup> This subsystem is not held by the 1.4 ms time constraint of the main processing subsystem. This is because the adaptive filter is executed locally on the main processing subsystem. The machine learning subsystem receives samples from the current 2 second period, decides on a pre-trained filter, and sends the corresponding filter coefficients back to the main processing subsystem. In order to get the most up to date filter coefficients from the machine learning subsystem, the bluetooth subsystem must be able to complete a full cycle of data transfer within an audio frame of 2 seconds. The amount of time sent is a balance between accuracy and speed. Since the purpose of  ****

<sup>6</sup> Since the machine learning subsystem is ran on a personal laptop, the system must be in range to communicate. The laptop's location will be restricted to inside our target room.

<sup>7</sup> In order to create a system that does not invade the privacy of the students and teachers in the room, the system will not store any sound data after it is used in processing. This will prevent the system from possibly storing private conversations that could be breached.

<sup>8</sup> In order to create a system that does not invade the privacy of the students and teachers in the room, the system must encrypt any sound data sent wirelessly. This will help protect sound data from being easily broadcasted and received outside of the room.

<sup>9</sup> The system has to communicate with the machine learning subsystem wirelessly in order to eliminate any need for wires or cables across the room. This removes physical obstructions in the walking area. This satisfies the accessibility ethics constraint.
...
The Arduino UNO R4 WiFi is able to transmit and receive in WiFi and Bluetooth 5 LE<sup>2</sup>. This is accomplished through the ESP32-S3-MINI[3]. Bluetooth 5.0 LE includes a PHY called LE 2M. This can support 2 Mb/s (2,000,000 bits per second). At the application layer, a data rate of up to approximately 1.4 Mb/s (1,400,000 bits per second) is possible[5]. Assuming the max 2 second delay lines of 187.5 kiB [6], each send will be sending 187.5 kiB (or 1,536,000 bits). Therefore, Bluetooth 5 LE  will be able to send a full delay line in 1.10 seconds (1.536 Mb / 1.4 Mbps), and receive the filter index bits (8 bits or less) in 5.714 ms or less<sup>4</sup>. This leaves 0.804 seconds for SPI communication.

spi here

Since the subsystem is able to communicate wirelessly, the physical extent of the system resides at the window preventing any obstruction of pathways<sup>9</sup>. The room's dimensions are 34x24x11 feet. A worst case scenario (lower corner to opposite upper corner) is a distance of 43.05 ft (13.122 meters). The ESP32-S3-MINI has a typical Bluetooth transmit power of ** dBm and a typical receiver sensitivity of ** dBm. Using Bluetooth's range estimation calculator[10], the ESP32-S3-MINI has an estimated range indoors of ** meters, which is greater than 13.122 meters<sup>6</sup>.
...
#### Encryption
The Arduino UNO R4 WiFi comes with the ** [1].  <sup>7</sup>. After a full cycle, all data will be written over<sup>8</sup>.

## BOM
| DEVICE                | Quantity | Price Per Unit | Total Price |
| --------------------- | -------- | -------------- | ----------- |
| Arduino UNO R4 WiFi   | 1        | $**         |             |
| 9V 1.5A AC/DC ADAPTER | 1        | $**         |             |
| Arduino Shield Pins   | 1        | $**         | $**     |
