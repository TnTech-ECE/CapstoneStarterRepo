...
In order to select the correct battery, the max voltage and current draw conditions of every subsystems component was collected. The battery chosen supplies 5200mAH of current which provides an extra 700mAH if needed as compared to the max current load demand of the entire robot. When modeling the load demand of the system, all the components of every subsytem can modeled as a resistor using Ohm's Law by using the max voltage and current draw of each component. To model dc to dc step down buck converter voltages in LT-SPice, a zener diode was used, and to apply the correct step down voltage from the main power supply, the forward bias voltage was chosen based off of the previous node voltage. 
