...
50.1 mV and 79.4 mV <sup>3</sup>. This will be put into the right side of the STEREO_IN defined in the main processor.   
...
The figure above shows an LT Spice version of the design. The OUT- pin in this case is set to a 1 kHz 70 mV peak sine wave, while the OUT + pin is set to a 1 kHz 70 mV peak sine wave. The output voltage is expected to be (OUT+) + (OUT-) which would be 1 V peak at the same frequency. The output voltage should have unity gain. 
