...
3. The vehicle detector system that will be used must be able to either manually tuned or automatically tuned to cover the given range of inductance values or greater [2].  When a vehicle passes over the loop, the frequency will change in the loop.  Due to frequency and inductance being directly proportional, the inductance of the loop will also change depending on the change in frequency.  In order to handle the change in frequency, whether the change is small or large, the inductance range must be large enough to detect any changes in the loop.  
